...
Symptom → cause → fix, fastest first. When unsure where you are, run `fairwave doctor` - it checks toolchains (kernel, docker, make, go), control-plane reachability, and RF-mode safety, and prints a pass/fail table.
...
| 2 | UE attach rejected with auth failure | Ki/OPc mismatch between SIM and HSS, or SIM profile/prefix mismatch | Re-issue the SIM and reload HSS; confirm IMSI prefix matches profile (`fairwave sim get <imsi>`); check `/v1/sims` state is `activated`, not `revoked` |
...
| 4 | `docker compose port` conflict (e.g. 8080/3000/27017 in use) | Host app already bound to a lab port | Remap host ports in `deploy/docker-compose.yml` (`"8080:8080"` → `"18080:8080"`), update `fairwave` CLI base URL, or stop the competing app |
...
| 6 | SIM rejected: "auth fail" at MME | Ki/OPc not loaded, SIM revoked, or rate-limit lockout | Check HSS has the IMSI (`docker compose logs open5gs`); `fairwave sim get <imsi>`; reload hook; confirm not on any block list |
| 7 | WireGuard handshake timeout | Wrong endpoint, NAT mapping stale, firewall blocks UDP/51820, keys mismatch after `node init` | `fairwave doctor`; `fairwave peer list`; verify UDP reachability (`nc -u`); enable `PersistentKeepalive`; re-join if public keys changed |
...
| 9 | Control plane auth failure (401/403) | Expired bearer token, revoked cert, wrong role for the action | Re-login (WebAuthn/TOTP); issue a fresh token (`fairwave token create`); check role vs endpoint requirement in [rest.md](../api/rest.md) |
...
```bash
fairwave doctor
docker compose -f deploy/docker-compose.yml logs --tail 100 enb ue open5gs
...
- Open an issue with the doctor output, logs (hashes fine - no credentials), and what you changed.
